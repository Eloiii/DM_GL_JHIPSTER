<template>
  <div>
    <h2 id="page-heading" data-cy="OrderHeading">
      <span v-text="$t('coopcycleApp.order.home.title')" id="order-heading">Orders</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('coopcycleApp.order.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'OrderCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('coopcycleApp.order.home.createLabel')"> Create a new Order </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && orders && orders.length === 0">
      <span v-text="$t('coopcycleApp.order.home.notFound')">No orders found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="orders">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('coopcycleApp.order.address')">Address</span></th>
            <th scope="row"><span v-text="$t('coopcycleApp.order.price')">Price</span></th>
            <th scope="row"><span v-text="$t('coopcycleApp.order.status')">Status</span></th>
            <th scope="row"><span v-text="$t('coopcycleApp.order.client')">Client</span></th>
            <th scope="row"><span v-text="$t('coopcycleApp.order.restaurant')">Restaurant</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="coopcycleApp.order.delete.question" data-cy="orderDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-order-heading" v-text="$t('coopcycleApp.order.delete.question', { id: removeId })">
