...
describe('Client e2e test', () => {
  ...
  const clientSample = { firstName: 'Angadrême', lastName: 'Dumas', phone: '+33 471059', address: 'Namibie' };
  ...
  describe('new Client page', () => {
    ...
    it('should create an instance of Client', () => {
      cy.get(`[data-cy="firstName"]`).type('Hubert').should('have.value', 'Hubert');

      cy.get(`[data-cy="lastName"]`).type('Vasseur').should('have.value', 'Vasseur');

      cy.get(`[data-cy="phone"]`).type('+33 384959').should('have.value', '+33 384959');

      cy.get(`[data-cy="address"]`).type('c calculating primary').should('have.value', 'c calculating primary');
