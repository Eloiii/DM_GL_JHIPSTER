...
describe('Order e2e test', () => {
  ...
  const orderSample = { address: 'copying', price: 53553, status: 'parse rich Lorraine' };
  ...
  describe('new Order page', () => {
    ...
    it('should create an instance of Order', () => {
      cy.get(`[data-cy="address"]`).type('Soft convergence seize').should('have.value', 'Soft convergence seize');

      cy.get(`[data-cy="price"]`).type('48895').should('have.value', '48895');

      cy.get(`[data-cy="status"]`).type('Tuna Barbade navigating').should('have.value', 'Tuna Barbade navigating');
